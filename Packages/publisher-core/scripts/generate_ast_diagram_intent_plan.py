"""Generate the Phase 3.4 AST Diagram Intent plan for product-owner review."""
...
OUTPUT_PATH = REPOSITORY_ROOT / "output" / "plans" / "ast_diagram_intent_v1.plan.json"
...
def main() -> None:
    ...
    plan = PublisherPlanner(registry).build_plan(
        source,
        PublicationRequest(
            request_id="request.ast_diagram_intent_v1_review",
    ...
    print(f"Diagram Intent Publication Plan: {OUTPUT_PATH}")
