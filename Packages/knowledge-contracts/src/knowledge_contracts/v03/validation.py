"""Draft 2020-12 validation for Knowledge JSON Version 0.3."""
...
from knowledge_contracts.v03.models import KnowledgeRecord


class KnowledgeSchemaError(ValueError):
    """Stable representation of a Version 0.3 schema failure."""
...
def knowledge_record_json_schema() -> dict[str, Any]:
    """Return the Version 0.3 schema with a category-template condition."""

    schema = KnowledgeRecord.model_json_schema(mode="validation")
    schema["$schema"] = "https://json-schema.org/draft/2020-12/schema"
    schema["$id"] = "https://bluprnt-lab.local/schemas/knowledge-record/0.3"
    schema["title"] = "BLUPRNT Lab Knowledge JSON Version 0.3"
    schema["allOf"] = [
        {
            "if": {
                "properties": {
            ...
            "then": {
                "properties": {
                    "category_content": {
                        "properties": {"template_id": {"const": "test_item_v0.3"}},
            ...
            "else": {
                "properties": {
                    "category_content": {
                        "properties": {"template_id": {"const": "generic_facts_v0.3"}},
                        "required": ["template_id"],
                    }
                }
            },
...
def validate_knowledge_record(
    ...
    """Validate both the public JSON Schema and cross-reference rules."""
