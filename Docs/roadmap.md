...
| Phase 5.21：Medical Review Governance & Approval Criteria Design | 完了・Phase 5.31へ実装済み |
...
| Phase 5.31：Medical Review Registry MVP | 実装完了・プロダクトオーナー確認待ち |
...
Phase 5.31ではKnowledge Registryから独立した追記型Medical Review Registryを追加しました。人がactive Claim、Evidence、Checklist、Knowledge全体を確認した結果を独立Review Versionへ固定し、現在のKnowledge/Claim/Evidence/Policy/期限との一致からFinal Approval Eligibilityを派生判定します。AIによるReviewと自動Approvalは行わず、実Knowledgeは`draft`のまま維持します。
...
- 最大4つのSearch Intentを1回の外部検索操作へまとめるQuery Builder
- Interactions APIとGoogle Search Groundingを内部へ閉じたGemini Search Provider
- Gemini回答本文を破棄し、外部Citation MetadataだけをRaw Evidenceへ変換するExtractor
- Domain許可リストに基づくJapan Official / International Official / Academic / Other分類
- UnknownをCに保つEvidence Level Policyと、独立したInformation Priority
- 既存Normalizer・Deduplicator・Ranker・Evidence Bundleへの接続
- MetadataだけのSearch Audit、Timeout、条件付きRetry最大1回、Query・Source上限
- 明示ボタン操作だけで実検索するWorkbench Preview
- [Gemini Grounded Evidence Search Provider MVP](gemini_grounded_evidence_search.md)
- [ADR-0021](adr/0021-gemini-grounded-search-as-citation-provider.md)

### Phase 5.26.1成果物

- `dsc_`／`dcs_`の安定IDを持つDiscovery Candidate Contract 1.0
- Claim、Evidence Bundle、Promotion、Registry、Approvalの5禁止フラグを`false`固定
- Discovery ProviderとFormal Evidence Providerの独立Interface
- 5つの誤接続先を停止する実行時Boundary Validation
- Gemini経路からEvidence Normalizer、Ranker、Bundle Builderを除外
- WorkbenchのDiscovery Results、正式Evidenceではない旨、専用Provider未接続ボタン
- Phase 5.26 API RouteとAudit履歴のMigration互換
- [Discovery Candidate Boundary](discovery_candidate_boundary.md)
- [ADR-0022](adr/0022-separate-discovery-from-formal-evidence.md)

### Phase 5.27成果物

- NCBI公式E-utilities ESearch → PMID → EFetch XMLのFormal Evidence経路
- PMID、Title、Authors、Journal、Date、Abstract、DOI、Publication Type、Language、MeSHの構造化
- 既存Evidence Contract、Deduplicator、Ranker、Evidence Bundleへの接続
- Discovery Candidateを変換せずPubMedへ再検索するHandoff
- 医療用語からのDirect PubMed Search
- 採用・除外・保留のHuman Selectionと追記専用Audit
- API Key有無に応じた内部Rate Limiter、最大20件、Retry最大1回
...
- Promotion前に確認する独立Knowledge Draft Contract 1.0
- Human Accepted / Edited Claimの本文・ID・順序を変えないLossless Assembler
- Human Selected Referenceを変えないReference Mapping
- 既存Claim完全一致コピーだけを許可する追跡可能Summary
- Category Section、Metadata、Completeness、Fingerprint
- Claim順、重複、Reference、Category、Fingerprint、Metadata、入力差分のValidator
- Validation失敗時の保存禁止と正式Registryから独立したFile Repository
- Workbench Knowledge Draft Preview、JSON / Markdown Export、Return to Authoring
