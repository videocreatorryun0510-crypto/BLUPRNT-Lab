...
def test_workbench_can_open_save_and_reopen_ferritin() -> None:
    ...
    assert starter["data"]["knowledge_id"] == "knw_10000013"
    assert starter["data"]["classification"]["term_type"] == (
        "laboratory_test_item"
    )
    assert starter["data"]["category_content"]["template_id"] == (
        "laboratory_test_item_v1.0"
    )
    ...
    assert saved["registry"]["knowledge"]["registry_key"] == (
        "laboratory_test.ferritin"
    )
    assert saved["registry"]["knowledge"]["knowledge_version"] == 1
    assert len(saved["registry"]["claims"]) == 11
...
def test_ferritin_save_does_not_scan_or_modify_existing_knowledge(
    tmp_path: Path,
) -> None:
    ...
    existing = registry.reconcile(
        ...
        actor="phase_5_11_test",
        note="回帰用Knowledgeを登録",
    ).record
    before = existing.model_dump(mode="json")

    ferritin = registry.reconcile(
        _record(),
        actor="phase_5_11_test",
        note="フェリチンを登録",
    ).record

    assert repository.view(ferritin.knowledge_id).relations == []
